...
public:
    ...
    explicit Bin(
            ...
            int32_t x1 = -1,
            int32_t y1 = -1
    ) : id(id1), w(w1), h(h1), maxw(maxw1), maxh(maxh1), x(x1), y(y1) {
        if (maxw == -1) {
            maxw = w;
        }
        if (maxh == -1) {
...
::std::ostream &operator<<(::std::ostream &os, const Bin &rhs) {
    return os << "Bin{ id: " << rhs.id << ", x: " << rhs.x << ", y: "
              << rhs.y << ", w: " << rhs.w << ", h: " << rhs.h
              << ", maxw: " << rhs.maxw << ", maxh: " << rhs.maxh
              << ", recount: " << rhs.refcount()<<" }";
}
...
public:
     ...
     */
    explicit Shelf(int32_t y1, int32_t w1, int32_t h1) : x_(0), y_(y1), w_(w1), h_(h1), wfree_(w1) {}
...
public:
     ...
     */
    explicit ShelfPack(int32_t w, int32_t h, const ShelfPackOptions &options = ShelfPackOptions{}) {
        width_ = w > 0 ? w : 64;
        height_ = h > 0 ? h : 64;
        maxId_ = 0;
    ...
    std::vector<Bin *> pack(std::vector<Bin> &bins, const PackOptions &options = PackOptions{}) {
        ...
        for (auto &bin : bins) {
            if (bin.w > 0 && bin.h) {
                Bin *allocation = packOne(-1, bin.w, bin.h);
                if (!allocation)
                    continue;
                if (options.inPlace) {
